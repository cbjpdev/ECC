...
/*---------------------------------------------------------------------------*/
  ...
  // Shamir trick init
void 
shamir_init(Point * pKey, Point * pointArray)
{
  uint16_t i;
  uint16_t j;
  ...
  for(i = 0; i < S_MAX; i++) {
	...
	ecc_add(&(pointArray[S_MAX+(S_MAX+1)*i]), &(pointArray[S_MAX+(S_MAX+1)*(i-1)]), baseP);
      }else{
...
/*---------------------------------------------------------------------------*/
  // Shamir trick
void 
shamir(Point * P0, NN_DIGIT * u1, NN_DIGIT * u2)
{
  int16_t i, tmp, tmp2;
  int8_t j, k;
  NN_DIGIT windex;
...
/*---------------------------------------------------------------------------*/
int 
ecdsa_init(Point * pKey)
{
  /* ecc should be initialized before generating public key */
  //ecc_init();
#ifdef SHAMIR_TRICK
  param = ecc_get_param();
...
/*---------------------------------------------------------------------------*/	
uint8_t 
ecdsa_verify(uint8_t *msg, uint16_t len, NN_DIGIT *r, NN_DIGIT *s, Point *Q)
{
  uint8_t sha1sum[20];
  NN_DIGIT sha1tmp[20/NN_DIGIT_LEN];
  SHA1Context ctx;
